...
library RLP {
    ...
    function toList(RLPItem memory item)
        ...
        returns (RLPItem[] memory)
    {
        require(isList(item), "RLP: iterator is not a list");
    ...
    function toBoolean(RLPItem memory item) internal pure returns (bool) {
        require(item.len == 1, "RLP: toBoolean invalid length");
        uint256 result;
        uint256 memPtr = item.memPtr;
        assembly {
    ...
    function toAddress(RLPItem memory item) internal pure returns (address) {
        ...
        require(item.len == 21, "RLP: toAddress invalid length");

        return address(uint160(toUint(item)));
    }

    function toUint(RLPItem memory item) internal pure returns (uint256) {
        require(item.len > 0 && item.len <= 33, "RLP: toUint invalid length");
    ...
    function toBytes(RLPItem memory item) internal pure returns (bytes memory) {
        require(item.len > 0, "RLP: toBytes invalid length");
